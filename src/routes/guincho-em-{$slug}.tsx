...
function CityPage() {
  ...
  return (
    <div className="container mx-auto px-4 py-10">
      ...
      {(city.slug === 'sao-paulo' || city.slug === 'sao-paulo-sp') && (
        <section className="mt-14">
          <h2 className="text-2xl font-bold md:text-3xl text-accent/90 mb-6">Socorro por Região de São Paulo</h2>
          <div className="grid gap-6 md:grid-cols-2">
            {SP_REGIONAL_FAQS.map((reg) => (
              <Card key={reg.zone} className="border-border/60 bg-muted/5">
                <CardContent className="p-6">
                  ...
                  <div className="mt-4 pt-4 border-t border-border/40">
                    <p className="text-[10px] uppercase font-bold text-muted-foreground mb-2">Principais Rodovias na região:</p>
                    <div className="flex flex-wrap gap-2">
                      {reg.links.map((link) => (
                        <Link 
                          ...
                          className="text-[11px] bg-secondary px-2 py-1 rounded hover:bg-primary hover:text-white transition-colors"
                        >
                          {link.name}
                        </Link>
                      ))}
