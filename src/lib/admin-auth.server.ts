...
export async function loginAdminSession(password: string) {
  ...
  if (!adminPass) {
    throw new Error("Admin password not configured. Set the ADMIN_PASSWORD environment variable on the server.");
  }
  if (!constantTimeEqual(password.trim(), adminPass)) {
...
export async function checkAdminSessionCookie() {
